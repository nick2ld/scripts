...
checkDatabaseEngine() {
  if [ "$OS_NAME" = "Debian" ]; then
    case $OS_VERSION in
      ...
      "13")
        case $DB_ENGINE in
          "MYSQL_8.4" | "MYSQL_8.0" | "MARIADB_11.8")
            echo "Database Engine: $DB_ENGINE"
          ;;
          *)
            die "Database Engine $DB_ENGINE not supported."
...
{
  apt -y upgrade
  apt -y install gnupg gpgv apt-transport-https debsums chrony redis-server
...
deb https://$ORIGIN/ $OS_CODE_NAME php-8.5
  ...
  if [ "$OS_NAME" = "Debian" ]; then
    case $OS_VERSION in
      ...
      "12")
        case $DB_ENGINE in
          "MYSQL_8.4")
            #See: https://docs.percona.com/percona-server/8.4/apt-repo.html
      ...
      "13")
        case $DB_ENGINE in
          ...
          "MARIADB_11.8")
            ...
            wget -qO /etc/apt/keyrings/mariadb-keyring.asc https://mariadb.org/mariadb_release_signing_key.asc
            rm -f /etc/apt/sources.list.d/mariadb.list /etc/apt/sources.list.d/mariadb.sources
            cat > /etc/apt/sources.list.d/mariadb.sources <<'EOF'
# MariaDB 11.8 repository list - created 2026-05-23 23:39 UTC
# https://mariadb.org/download/
X-Repolib-Name: MariaDB
Types: deb
# deb.mariadb.org is a dynamic mirror if your preferred mirror goes offline. See https://mariadb.org/mirrorbits/ for details.
# URIs: https://deb.mariadb.org/11.8/debian
URIs: https://mirror.truenetwork.ru/mariadb/repo/11.8/debian
Suites: trixie
Components: main
